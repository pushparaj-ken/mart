...
const Featuredcontent = require('../../model/featuredcontent')
const Services = require('../../services/excel')
const imageUpload = require('../../services/image_upload.service');
const foldername = "Featuredcontents";


const Addfeaturedcontent = catchAsync(async (req, res) => {
  try {
    const values = req.body;
    const files = req.files;
    let image_new = "";
    if (files.featured_image) {
      let { buffer, originalname } = files.featured_image[0];
    ...
    Featuredcontent(Data).save().then((Result) => {
      res.send({
        success: true,
        code: 200,
...
const Updatefeaturedcontent = catchAsync(async (req, res) => {
  try {
    const values = req.body;
    const files = req.files;
    const params = req.params;
    if (params.id != '' && params.id != null && params.id != undefined) {
      let image_new = values.featured_image;
      if (files.featured_image) {
        let { buffer, originalname } = files.featured_image[0];
        let image = await imageUpload.upload(buffer, originalname, foldername);
        console.log(image.Location);
        values.featured_image = image.Location;
      } else {
        values.featured_image = image_new;
      }
      ...
      Featuredcontent.findOneAndUpdate(query, changes).lean().exec().then((UpdateStatus) => {
        res.send({
          success: true,
          code: 200,
          status: "Data Saved Success",
      ...
      res.send({
        code: 201,
        success: false,
        status: "Id required to Update featuredcontent.",
        timestamp: new Date()
      });
    }
  } catch (err) {
...
const Deletefeaturedcontent = catchAsync(async (req, res) => {
  try {
    const params = req.params;
    const values = req.body
    if (params.id != '' && params.id != null && params.id != undefined) {
...
const Listfeaturedcontent = catchAsync(async (req, res) => {
  try {
    ...
    const totalpage = await Featuredcontent.countDocuments(query)
    const pipeline = [];
    let skip = 0;
    ...
    const Results = await Featuredcontent.aggregate(pipeline);
    if (Results.length > 0) {
      const Records = Results.map(({ id, is_verified, status, ...rest }) => rest);
      let filePath = await Services.ExportCommonExcel(Records, "Featuredcontent_list");
      var fullPublicUrl = process.env.fullPublicUrl;
      let downloadurl = `${fullPublicUrl}${filePath}`
      res.send({
        success: true,
        code: 200,
        status: "featuredcontent Lists retrieved successfully",
